...
public class Main{
    // Hardware Resources
    static String mainMemory[][] = new String[300][4], instructionRegister[] = new String[4],
    generalPurposeReg[] = new String[4];
    ...
    static File inputFile = new File("ip.txt");
    ...
    static int getRA(int VA) {
        ...
        // Eventually return null in case virtual address doesn't match with any of the
        // VRs in page table
        return -1;
    }
    ...
    static void mapAddress(int VA) {
        ...
        VA = (VA / 10);
        for (int j = 0; j < 27; j++) { // we just search in 27 blocks (frames) because last tree ones are just
                                       // allocated for page table
            try {
                // The following checks and if check is successful then it means that the block
                // has an item and it is full
                // Otherwise this check will cause a null exception which means that the block
                // is a hole actually
    ...
    static void read() {
        ...
        // Read next (data) card from input file in memory locations IR [3,4] through IR
        // [3,4] +9
        instructionRegister[3] = "0";
        int memoryAddress = 0;
        for (int i = 2; i < 4; i++)
        memoryAddress = memoryAddress * 10 + Integer.parseInt(String.valueOf(instructionRegister[i])); // Getting                                                                             // card in.
        M = memoryAddress;
    ...
    // Write function
    static void write() {
        // Write one block (10 words of memory) from memory locations IR [3,4] through
        // IR [3,4] + 9 to output file
        int memoryAddress = 0;
        ...
        // print("Write() IR:-
        // "+instructionRegister[0]+instructionRegister[1]+instructionRegister[2]+instructionRegister[3]);//test...

        // Extracting VA from IR
        for (int i = 2; i < 4; i++)
            memoryAddress = memoryAddress * 10 + Integer.parseInt(String.valueOf(instructionRegister[i]));
        // print("I will start writing the following block: -" + memoryAddress);
        // //Test...
    ...
    static void terminate(int code) {
        ...
        try {
            ...
            if (code == 7) {
                // System.out.println("Time Limit Exceed and Opcode
                // Error.\tterminate(7)");//test...
                ErrorMessage = "";
                ErrorMessage = ErrorMessage
            ...
            if (code == 8) {
                // System.out.println("Time Limit Exceed and Operand
                // Error.\tterminate(6)");//test...
                ErrorMessage = "";
                ErrorMessage = ErrorMessage
    ...
    static void executeUserProgram() {
        ...
        while (continueExecution) {
            ...
            for (int i = 0; i < 4; i++)
                instructionRegister[i] = mainMemory[instructionCounter][i]; // loading to instructions to Instruction   // Register
            instructionCounter++;
            ...
            for (int i = 0; i < 2 && instructionRegister[i] != null; i++)
                instruction = instruction.concat(instructionRegister[i]); // getting the instruction itself
            try {
                for (int i = 2; i < 4 && instructionRegister[i] != null; i++)
                    memoryAddress = (memoryAddress * 10) + Integer.parseInt(instructionRegister[i]); // getting the main
                                                                                                     // memory reference    // Address
            } catch (Exception e) {
                // Catching Operand Error
                // print("Operand Error caught the operand: "+
                // instructionRegister[2]+instructionRegister[3]);//test...
                // terminate(5); //test...
                PI = 2;
                MOS();
                continueExecution = false;
            ...
            if (instruction.equals("LR")) {
                ...
                if (!pageFault) {
                    try {// We put another check maybe there is internal fragmentation
                        if (mainMemory[memoryAddress][0].equals("arg0"))
                            ;
                        for (int i = 0; i < 4; i++) {
                            if (generalPurposeReg[i] == mainMemory[memoryAddress][i])
                                toggle = true;
                            else {
                                toggle = false;
                                break;
                ...
                if (toggle)
                    instructionCounter = memoryAddress;
        ...
        try {
            ...
            while (loadBuffer != null) {
                ...
                if (loadBuffer.substring(0, 4).equals("$AMJ")) {
                    ...
                    // ------------------------------------------------------------------------
                    for (int i = 4; i < 8; i++)
                        JID = JID * 10 + Integer.parseInt(String.valueOf(loadBuffer.charAt(i)));
                    for (int i = 8; i < 12; i++)
                        TTL = TTL * 10 + Integer.parseInt(String.valueOf(loadBuffer.charAt(i)));
                    for (int i = 12; i < 16; i++)
                        TLL = TLL * 10 + Integer.parseInt(String.valueOf(loadBuffer.charAt(i)));
                    ...
                    // if first four characters of the the next line is not equal to $DTA the keep
                    // loading th line to main memory
                    while (true) {
                        controlsToBeLoaded = controlsToBeLoaded.concat(loadBuffer);
                        loadBuffer = reader.readLine();

                        // if the length of the load buffer is less than 4 then it will raise exception
                        // which is needed considered
                        if (loadBuffer.length() >= 4) {
                            if (loadBuffer.substring(0, 4).equals("$DTA"))
                                break;
                        }
                    }
                    ...
                    // After loading the control cart to main memory we need to take care of the
                    // data part.
                    // ---------------------------------------------------------------------------------------------
                    DTApart = "";
                    DTAPointer = 0;
                    ...
                    while (true) {
                        loadBuffer = reader.readLine();
                        if (loadBuffer.length() >= 4) {
                            if (loadBuffer.substring(0, 4).equals("$END"))
                                break;
                        }
